...
PURE math — no heavy deps, so it is unit-testable in the light venv and shared by
both the CPU (moviepy) and GPU (torch) compositors.
...
def normalize_screen_keyframes(keyframes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Canonicalize screen-zoom keyframes: each has t, zoom (1.0 = full/overview, 2.0 =
    2x into focus), focus [nx, ny] (normalized centre of the zoom), ease."""
    out = []
    for kf in keyframes:
        k = dict(kf)
        k["t"] = float(k["t"])
        k["zoom"] = max(1.0, float(k.get("zoom", 1.0)))
        f = k.get("focus", [0.5, 0.5])
        k["focus"] = [float(f[0]), float(f[1])]
        k["ease"] = k.get("ease", "inout")
        out.append(k)
    out.sort(key=lambda k: k["t"])
    return out
...
def sample_screen(keyframes: list[dict[str, Any]], t: float,
                  sw: int, sh: int) -> tuple[int, int, int, int]:
    """Source-pixel crop rect (x, y, w, h) of the screen at time t. Aspect is preserved
    (so the display zone shape never changes); higher zoom = smaller crop = bigger
    content. Empty/None keyframes => full screen."""
    if not keyframes:
        return 0, 0, sw, sh
    kfs = keyframes

    def zf(kf):
        return kf["zoom"], kf["focus"][0], kf["focus"][1]
    ...
    cw = sw / z
    ch = sh / z
    cx = min(max(fx * sw - cw / 2, 0.0), sw - cw)
    cy = min(max(fy * sh - ch / 2, 0.0), sh - ch)
    return int(round(cx)), int(round(cy)), max(2, int(round(cw))), max(2, int(round(ch)))
