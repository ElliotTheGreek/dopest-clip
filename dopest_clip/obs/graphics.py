"""SVG overlay graphics: parametric built-in pointer shapes whose ANCHOR (the point
that lands on the screen target, e.g. an arrow's tip) is defined so the agent can aim
them at located screen coordinates.

Each builder returns (svg_string, anchor) where anchor is [ax, ay] in 0..1 of the
graphic's own box -- the point placed at the keyframe position. The builders are PURE
string producers (no deps), so they are fully unit-testable in the light venv.

Rasterization (``render_svg``) lazily imports resvg-py / numpy / PIL — install with:
``pip install dopest-clip[graphics]`` (PIL is a base dep; numpy ships with [matting]).
"""
...
def render_svg(svg: str, width: int):  # type: ignore[no-untyped-def]
    ...
    Lazy-imports resvg-py (and numpy). Install with: pip install dopest-clip[graphics]
    (plus dopest-clip[matting] for numpy)."""
    import io
    ...
    try:
        ...
        raise RuntimeError(
            "SVG overlay rasterization needs resvg-py. "
            "Install the graphics extra with: pip install dopest-clip[graphics]"
        ) from e
    try:
        import numpy as np
    except ImportError as e:  # noqa: BLE001
        raise RuntimeError(
            "SVG overlay rasterization needs numpy. "
