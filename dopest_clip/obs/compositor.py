...
camera's position and scale from a keyframe timeline, and write a final mp4.
...
moviepy / numpy / PIL / rembg are imported LAZILY inside ``compose`` and its helpers,
so the OBS-control half of the package (and this module's import) works without them.
Install the compositing extra with: ``pip install dopest-clip[matting]``.
"""
...
def compose(
    ...
    try:
        ...
        raise RuntimeError(
            "compose() needs moviepy. Install the compositing extra with: "
            "pip install dopest-clip[matting]"
        ) from e
