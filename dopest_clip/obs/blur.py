...
The fade-amplitude / keyframe params are PURE (no deps); the pixel work (mask
build + Gaussian blur) lazily imports numpy/PIL. Install with:
``pip install dopest-clip[matting]`` (PIL is a base dep; numpy ships with [matting]).
"""
...
def make_processor(blurs: list[dict[str, Any]], fw: int, fh: int,
    ...
    blur specs to the screen frame at time t. Lazy-imports numpy/PIL."""
    import numpy as np
